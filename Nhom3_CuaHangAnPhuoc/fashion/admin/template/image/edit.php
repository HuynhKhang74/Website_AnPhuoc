<form action="index.php?page=image&action=save" method="post" accept-charset="utf-8" enctype="multipart/form-data">
    ...
    <table class="table" style="width: 600px; margin: auto;">
        ...
        <tr>
            <th>Tiêu đề</th>
            <td>
                <input type="text" name="title" placeholder="Nhập tiêu đề" value="<?= $image['title'] ?? '' ?>" >
            </td>
        </tr>
        <tr>
            <th>Ảnh Hiện Tại : </th>
            ...
            <td>
                <select name="type" id="type">
                    <?php foreach($types as $key => $type) {  ?>
                        <option <?= isset($image['type']) && $image['type'] == $key ? 'selected' : '' ?>  value="<?= $key ?>"><?= $type ?></option>
                    <?php } ?>
                </select>
            </td>
        </tr>
        <tr>
            <td colspan="2" class="center">
                ...
                <a href = "index.php?page=image&action=list">Quay lại</a>
            </td>
        </tr>
    </table>
    <script>
